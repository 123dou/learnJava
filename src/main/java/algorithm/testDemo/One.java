...
public class One {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(new One().threeSum(arr));
    }
    ...
    public boolean isValidBST(TreeNode root) {
        ...
        boolean isOrder = isValidBST(root.left);
        if (!isOrder) {
            return false;
        }
        if (isMin) {
    ...
    private TreeNode CreateBST(int[] nums, int left, int right) {
        ...
        int curr = left + ((right - left) >> 1);
        TreeNode node = new TreeNode(nums[curr]);
        node.left = CreateBST(nums, left, curr - 1);
        node.right = CreateBST(nums, curr + 1, right);
        return node;
    ...
    public List<List<Integer>> threeSum(int[] nums) {
        ...
        if (zero > 0) {
            for (Integer i : negSet) {
                if (posSet.contains(-i)) {
                    res.add(Arrays.asList(0, i, -i));
                }
            }
        }
        ...
        int[] arr1 = new int[neg.size()];
        int[] arr2 = new int[pos.size()];
...
//        }
        ...
        for (int i = 0; i < neg.size(); i++) {
            if (i > 0 && neg.get(i) == neg.get(i - 1)) {
                continue;
            }
            findTwo(pos, -neg.get(i));
        }

        for (int i = 0; i < pos.size(); i++) {
            if (i > 0 && pos.get(i) == pos.get(i - 1)) {
                continue;
            }
            findTwo(neg, -pos.get(i));
        }
    ...
    private void findTwo(List<Integer> l, int target) {
        int lo = 0;
        int hi = l.size() - 1;
        while (lo < hi) {
            if (l.get(lo) + l.get(hi) == target) {
                res.add(Arrays.asList(-target, l.get(lo), l.get(hi)));
                lo++;
                hi--;
            ...
            while (lo < hi && lo > 0 && l.get(lo) == l.get(lo - 1)) {
                lo++;
            }

            while (hi > 0 && hi < l.size() - 1 && l.get(hi) == l.get(hi + 1)) {
                hi--;
            }
        }
    }

    private void findTwo(int[] arr, int target) {
        int lo = 0;
        int hi = arr.length - 1;
        for (int i = 0; i < arr.length; i++) {
            ...
            while (lo < hi && lo > 0 && arr[lo] == arr[lo - 1]) {
                lo++;
            }

            while (lo < hi && hi < arr.length - 1 && arr[hi] == arr[hi + 1]) {
                hi--;
            }
        }
